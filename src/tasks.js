...
const execCheck = path =>
  (!!(fs.existsSync(path) && fs.lstatSync(path).isDirectory()));

const execRename = path =>
  (execCheck(path) ? existsPath(path).then(data => execRename(data.name)) : path);

const cloneGit = (user, repository, changed = null) =>
  execa.stdout('git', ['clone', `https://github.com/${user}/${repository}.git`, `${changed || ''}`]);

const skipInstall = path => !fs.existsSync(`${path}/package.json`);

const installPackages = path =>
  execa.stdout('npm', ['install'], { cwd: path });
